...
public class JHS_UltraJunkhubStart extends CustomStart {
    ...
    public void execute(InteractionDialogAPI dialog, Map<String, MemoryAPI> memoryMap) {
        ...
        moduleDmodlist.add("comp_armor");
        moduleDmodlist.add("comp_hull");
        moduleDmodlist.add("faulty_grid");
        moduleDmodlist.add("comp_structure");

        moduleDmodlist.add("glitched_sensors");
        moduleDmodlist.add("faulty_grid");
        moduleDmodlist.add("fragile_subsystems");
        ...
        data.addScript(() -> {
            ...
            for (FleetMemberAPI member : fleet.getFleetData().getMembersListCopy()) {
                if (member.getHullId().contentEquals(StartingHullID)) {
                    ...
                    ShipVariantAPI leftModule = member.getVariant().getModuleVariant("WS0001").clone();
                    leftModule.setSource(VariantSource.REFIT);
                    leftModule.setHullVariantId(Misc.genUID());

                    leftModule.addPermaMod(HullMods.INTEGRATED_TARGETING_UNIT);

                    ShipVariantAPI rightModule = member.getVariant().getModuleVariant("WS0002").clone();
                    rightModule.setSource(VariantSource.REFIT);
                    rightModule.setHullVariantId(Misc.genUID());

                    rightModule.addPermaMod(HullMods.INTEGRATED_TARGETING_UNIT);
                    ...
                    ShipVariantAPI shieldModule = member.getVariant().getModuleVariant("WS0009").clone();
                    shieldModule.setSource(VariantSource.REFIT);
                    shieldModule.setHullVariantId(Misc.genUID());

                    shieldModule.addPermaMod("JHS_Unstable_Shield");

                    member.getVariant().setModuleVariant("WS0001", leftModule);
                    member.getVariant().setModuleVariant("WS0002", rightModule);
                    member.getVariant().setModuleVariant("WS0009", shieldModule);
