...
public class JHS_Core_RestoreOrUpgradeAvailable extends BaseCommandPlugin {
    ...
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Token> params, Map<String, MemoryAPI> memoryMap) {
        ...
        if (restore && upgrade) {
            memoryMap.get(MemKeys.GLOBAL).set("$JHS_core_RestoreOrUpgradeStr", "Restore or upgrade", 0);
            if (targetMember != null) {
                memoryMap.get(MemKeys.GLOBAL).set("$JHS_core_RestoreOrUpgradeTooltip", "Explore options to clear d-mods from " + targetMember.getShipName() + " or to apply upgrades to its hull.", 0);
            }
        } else if (restore) {
            memoryMap.get(MemKeys.GLOBAL).set("$JHS_core_RestoreOrUpgradeStr", "Restore", 0);
            if (targetMember != null) {
                memoryMap.get(MemKeys.GLOBAL).set("$JHS_core_RestoreOrUpgradeTooltip", "Explore options to clear d-mods from " + targetMember.getShipName() + ".", 0);
            }
        } else if (upgrade) {
            memoryMap.get(MemKeys.GLOBAL).set("$JHS_core_RestoreOrUpgradeStr", "Upgrade", 0);
            if (targetMember != null) {
                memoryMap.get(MemKeys.GLOBAL).set("$JHS_core_RestoreOrUpgradeTooltip", "Explore options to apply upgrades to " + targetMember.getShipName() + ".", 0);
            }
        }
    ...
    protected boolean restoreAvailable(String ruleId, InteractionDialogAPI dialog, List<Token> params, Map<String, MemoryAPI> memoryMap) {
        ...
        memoryMap.get(MemKeys.GLOBAL).set("$JHS_RestoreShip", targetMember.getShipName(), 0);
        if (numDMods > 1) {
            memoryMap.get(MemKeys.GLOBAL).set("$JHS_RestoreModsStr", "one random", 0);
        } else {
            memoryMap.get(MemKeys.GLOBAL).set("$JHS_RestoreModsStr", "the last", 0);
        }
        long startingDMods = 7;
        if (Global.getSector().getMemoryWithoutUpdate().contains("$JHS_StartingDMods")) {
            startingDMods = Global.getSector().getMemoryWithoutUpdate().getLong("$JHS_StartingDMods");
        }
        if (numDMods == 0) {
            return false;
        }
    ...
    protected boolean anyUpgradeAvailable(String ruleId, InteractionDialogAPI dialog, List<Token> params, Map<String, MemoryAPI> memoryMap) {
        ...
        for (UpgradeType upgrade : possibleUpgrades) {
            memoryMap.get(MemKeys.GLOBAL).set("$JHS_Upgrade" + upgrade.id + "CostStr", Misc.getWithDGS(upgrade.credits), 0);
            memoryMap.get(MemKeys.GLOBAL).set("$JHS_Upgrade" + upgrade.id + "SwapCostStr", Misc.getWithDGS(upgrade.swapCredits), 0);
        }
    ...
    public static enum UpgradeType {
        ...
        U1_FIXED_ALL_WINGS("U1", 100000, 0),
        U2_FIXED_DRONE_LAUNCHERS("U2", 200000, 0),
        U3_FIXED_FLEET_GANTRY("U3", 175000, 0),
        U4_FIXED_WEAPON_SLOTS_40_OP("U4", 175000, 0),
        U5_FIXED_TARGETING_CORE("U5", 150000, 0),

        //ADDITIONAL PERMA UPGRADES
        P1_ADDED_LOGISTICS("P1", 200000, 0),
        P2_ADDED_VOIDSHIP_COMMAND("P2", 125000, 0),

        //SWAPPABLE UPGRADES
        W1_CHOOSE_MODERNISED_WEAPON_SLOTS("W1", 450000, 250000),
        W2_CHOOSE_MODERNISED_WING_SLOTS("W2", 450000, 450000),

        M1_CHOOSE_CARRIER_LEFT_MODULE("M1", 600000, 600000),
        M2_CHOOSE_LOGISTIC_RIGHT_MODULE("M2", 600000, 600000),
        M3_CHOOSE_BALANCED_MODULES("M3", 600000, 600000),

        S1_CHOOSE_BURN_DRIVE_SYSTEM("S1", 450000, 450000),
        S2_CHOOSE_OFFENCE_FOCUS_SYSTEM("S2", 450000, 450000),
        S3_CHOOSE_TARGETING_FEED_SYSTEM("S3", 450000, 450000);
        ...
        public static EnumSet<UpgradeType> getCurrentUpgrades(ShipVariantAPI variant) {
            ...
            if (name.startsWith(token + "_s1")) {
                token += "_s1";
                upgrades.add(S1_CHOOSE_BURN_DRIVE_SYSTEM);
            } else if (name.startsWith(token + "_s2")) {
                token += "_s2";
                upgrades.add(S2_CHOOSE_OFFENCE_FOCUS_SYSTEM);
            } else if (name.startsWith(token + "_s3")) {
                token += "_s3";
                upgrades.add(S3_CHOOSE_TARGETING_FEED_SYSTEM);
            } else {
                return upgrades;
            }
        ...
        public static EnumSet<UpgradeType> getPossibleUpgrades(ShipVariantAPI variant) {
            ...
            if (name.startsWith(token + "_s1")) {
                token += "_s1";
                upgrades.remove(S1_CHOOSE_BURN_DRIVE_SYSTEM);
            } else if (name.startsWith(token + "_s2")) {
                token += "_s2";
                upgrades.remove(S2_CHOOSE_OFFENCE_FOCUS_SYSTEM);
            } else if (name.startsWith(token + "_s3")) {
                token += "_s3";
                upgrades.remove(S3_CHOOSE_TARGETING_FEED_SYSTEM);
            }
        ...
        public static ShipHullSpecAPI getHullSpec(EnumSet<UpgradeType> upgrades) {
            ...
            if (upgrades.contains(S1_CHOOSE_BURN_DRIVE_SYSTEM)) {
                name += "_s1";
            } else if (upgrades.contains(S2_CHOOSE_OFFENCE_FOCUS_SYSTEM)) {
                name += "_s2";
            } else if (upgrades.contains(S3_CHOOSE_TARGETING_FEED_SYSTEM)) {
                name += "_s3";
            }
