...
// Mostly same code as in RestoreShip cmd
public class JHS_Core_RestoreEnoughCredits extends BaseCommandPlugin {
    ...
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Token> params, Map<String, MemoryAPI> memoryMap) {
        ...
        if (Global.getSector().getMemoryWithoutUpdate().contains("$JHS_core_startingDmods")) {
            startingDMods = Global.getSector().getMemoryWithoutUpdate().getLong("$JHS_core_startingDmods");
        }
        if (numDMods == 0) {
            return false;
        }
